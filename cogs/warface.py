...
# Commands
    ...
    async def браво(self, ctx, arg):
        r = requests.get("http://api.warface.ru/user/stat/?name={}&server=2".format(arg))
        response_json = r.json()
        if 'message' in response_json:
            embbr = discord.Embed(color=discord.Color.light_grey(), title = "{}".format(response_json['message']))
    ...
    async def альфа(self, ctx, arg):
        r = requests.get("http://api.warface.ru/user/stat/?name={}&server=1".format(arg))
        response_json = r.json()
        if 'message' in response_json:
            embbr = discord.Embed(color=discord.Color.light_grey(), title = "{}".format(response_json['message']))
            await ctx.send(embed=embbr)
    ...
    async def чарли(self, ctx, arg):
        r = requests.get("http://api.warface.ru/user/stat/?name={}&server=3".format(arg))
        response_json = r.json()
        if 'message' in response_json:
            embbr = discord.Embed(color=discord.Color.light_grey(), title = "{}".format(response_json['message']))
            await ctx.send(embed=embbr)
